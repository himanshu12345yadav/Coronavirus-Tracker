...
const GraphTracker = (props) => {
  ...
  useEffect(() => {
    ...
    if (!renderFlag) {
      ...
      document.querySelector(".graph-feedback.invalid-feedback").innerText =
  ...
  const add_country_handler = (event) => {
    ...
    try {
      ...
      if (cty_name.length === 0) {
        ...
        window.lineChart.data.datasets.some((obj) => obj.label === cty_name)
      ) {
        let error = new Error(
        ...
        throw error;
      } else if (!country_names.some((country) => cty_name === country.name)) {
  ...
  const weeks_change_handler = (event) => {
    ...
    window.lineChart.data.datasets.forEach((dataset, index) => {
      dataset.data = window.complete_requested_data[index].data.slice(
        window.slice_count
      );
  ...
  const fetchData = async (country, chosen_weeks) => {
    ...
    let url = `https://api.covid19api.com/total/country/${country}/status/confirmed?from=${from_date}T00:00:00Z&to=${to_date}T00:00:00Z`;
  ...
  const parseData = (country, weeks) =>
    new Promise(async (resolve, reject) => {
      try {
        ...
        let dailyData = [];
        let dateData = [];
        let prevCases = rawDailyData[0].Cases;
        rawDailyData.forEach((item) => {
          dailyData.push(item.Cases - prevCases);
          prevCases = item.Cases;
          dateData.push(formatDate(item.Date.split("T")[0].split("-")));
        });
        dailyData.shift();
        dateData.shift();
        resolve([dailyData, dateData]);
      } catch (error) {
        reject(error);
      }
    });
  ...
  const renderGraph = async (
    ...
    let [dailyData, dateData] = await parseData(country_name, weeks_count);
    try {
    ...
    window.complete_requested_data = window.lineChart.data.datasets.map(
      (item) => Object.assign({}, item)
    );
    window.complete_requested_labels = Array.from(window.lineChart.data.labels);
  };
  ...
  const addGraphs = async (country_name, weeks) => {
    ...
    try {
      ...
      var colorName =
        colorNames[window.lineChart.data.datasets.length % colorNames.length];
      var newColor = window.chartColors[colorName];
      let [dailyData] = await parseData(country_name, weeks);
      var newsliceDataset = {
        label: country_name,
        ...
        data: dailyData.slice(
          typeof window.slice_count === undefined ? 0 : window.slice_count
        ),
      };
      var newDataset = {
        label: country_name,
      ...
      window.complete_requested_data.push(newDataset);

      window.complete_requested_data = window.complete_requested_data.map(
        (item) => Object.assign({}, item)
      );
